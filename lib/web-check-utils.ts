...
const TECH_PATTERNS = {
  frameworks: [
    ...
    {
      name: "Angular",
      patterns: [/ng-version/, /ng-app/, /_angular/, /angular\.js/i, /\[ng-/],
      versionPattern: /ng-version="([^"]+)"/,
      confidence: "high",
    },
    {
      name: "Svelte",
      patterns: [/svelte/i, /class="svelte-/, /data-svelte/],
      confidence: "medium",
    },
    {
      name: "Remix",
      patterns: [/__remix/, /remix\.js/i, /"context":{"remix"/],
      confidence: "high",
    },
    {
      name: "Astro",
      patterns: [/astro-/, /data-astro-/, /\.astro/],
      confidence: "high",
    },
    {
      name: "SvelteKit",
      patterns: [/__sveltekit/, /_app\/immutable/, /sveltekit/i],
      confidence: "high",
    },
    {
      name: "Gatsby",
      patterns: [/___gatsby/, /gatsby-/, /public\/static\/[a-f0-9]+\/[a-f0-9]+\.js/],
      confidence: "high",
    },
  ],
  cms: [
    ...
    {
      name: "Joomla",
      patterns: [/\/components\/com_/, /Joomla!/, /\/media\/jui\//],
      confidence: "high",
    },
    {
      name: "Shopify",
      patterns: [/cdn\.shopify\.com/, /\/wpm@/, /Shopify\.theme/, /shopify-section/],
      confidence: "high",
    },
    {
      name: "Webflow",
      patterns: [/webflow\.com/, /data-wf-page/, /\.webflow\.io/],
      confidence: "high",
    },
    {
      name: "Wix",
      patterns: [/static\.wixstatic\.com/, /wix\.com/, /parastorage\.com/],
      confidence: "high",
    },
    {
      name: "Squarespace",
      patterns: [/squarespace/i, /\.sqsp\./, /static1\.squarespace/],
      confidence: "high",
    },
    {
      name: "Ghost",
      patterns: [/ghost\.io/, /ghost-/, /powered by Ghost/i],
      confidence: "high",
    },
    {
      name: "Contentful",
      patterns: [/contentful\.com/, /\/\/images\.ctfassets\.net/],
      confidence: "high",
    },
    {
      name: "Strapi",
      patterns: [/X-Powered-By: Strapi/i, /strapi/i],
      confidence: "medium",
    },
  ],
  analytics: [
    ...
    },
    {
      ...
      patterns: [/googletagmanager\.com\/gtm\.js/, /GTM-[A-Z0-9]+/],
      confidence: "high",
    },
    {
      name: "Facebook Pixel",
    ...
    {
      name: "Matomo",
      patterns: [/matomo\.js/, /piwik\.js/, /_paq\.push/],
      confidence: "high",
    },
    {
      name: "Plausible",
...
function detectMetaGenerators(html: string): Technology[] {
  const technologies: Technology[] = [];
  const generatorMatches = html.match(
...
function detectFromHeaders(headers: Record<string, string>): Technology[] {
  ...
  const originHeaders = ["x-origin-server", "x-backend-server", "x-served-by", "x-upstream-server"];
...
export function detectTechnologies(
  html: string,
  headers: Record<string, string>
): Array<{ name: string; version?: string; confidence: "high" | "medium" | "low" }> {
...
export function checkVulnerabilityIndicators(
  html: string,
  url: string
): {
  category: string;
...
export function calculateSecurityScore(data: {
  ...
  const sensitiveFiles =
    data.endpoints?.commonFiles?.filter(
      (f: any) => f.path.includes(".env") || f.path.includes(".git") || f.path.includes("backup")
    ) || [];
...
export function generateRecommendations(findings: any): Array<{
  ...
  if (findings.exposedData?.apiKeys?.length > 0) {
    recommendations.push({
      ...
      description:
        "API keys were found in your source code. These should never be exposed client-side.",
      action:
        "Rotate all exposed API keys immediately and move them to environment variables on the server.",
      resources: ["https://cheatsheetseries.owasp.org/cheatsheets/Key_Management_Cheat_Sheet.html"],
    });
  }
